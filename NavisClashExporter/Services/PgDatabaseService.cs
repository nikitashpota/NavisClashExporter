...
namespace NavisClashExporter.Services
{
    public class PgDatabaseService : IDisposable
    {
        ...
        public PgDatabaseService(string connectionString) { _cs = connectionString; }
        ...
        public List<DirectoryModel> GetAllDirectories()
        {
            ...
            var conn = new NpgsqlConnection(_cs); conn.Open();
            var cmd = new NpgsqlCommand("SELECT id,code,created_at FROM directories ORDER BY code", conn);
            var r = cmd.ExecuteReader();
            while (r.Read())
                list.Add(new DirectoryModel { Id = r.GetInt32(0), Code = r.GetString(1), CreatedAt = r.GetDateTime(2) });
            return list;
        }
        ...
        public DirectoryModel CreateDirectory(string code)
        {
            ...
            return new DirectoryModel { Id = r.GetInt32(0), Code = code, CreatedAt = r.GetDateTime(1) };
        }

        public void UpdateDirectory(int id, string code)
        {
            var conn = new NpgsqlConnection(_cs); conn.Open();
            var cmd = new NpgsqlCommand("UPDATE directories SET code=@c WHERE id=@id", conn);
            cmd.Parameters.AddWithValue("c", code); cmd.Parameters.AddWithValue("id", id);
            cmd.ExecuteNonQuery();
        }
        ...
        {
            var conn = new NpgsqlConnection(_cs); conn.Open();
            var cmd = new NpgsqlCommand("DELETE FROM directories WHERE id=@id", conn);
        ...
        private NavisworksProjectModel MapProject(NpgsqlDataReader r)
        {
            ...
            if (!r.IsDBNull(6))
                p.Directory = new DirectoryModel { Id = r.GetInt32(6), Code = r.GetString(7), CreatedAt = r.GetDateTime(8) };
            return p;
        }
        ...
        public List<NavisworksProjectModel> GetAllProjects()
        {
            ...
            var conn = new NpgsqlConnection(_cs); conn.Open();
            var cmd = new NpgsqlCommand(ProjectSql + " ORDER BY p.name", conn);
            var r = cmd.ExecuteReader();
        ...
        {
            var conn = new NpgsqlConnection(_cs); conn.Open();
            var cmd = new NpgsqlCommand(ProjectSql + " WHERE p.name=@n", conn);
        ...
        public void UpdateProject(int id, string name, string nwfPath, int? directoryId)
        {
            ...
            cmd.Parameters.AddWithValue("n", name); cmd.Parameters.AddWithValue("p", nwfPath);
            cmd.Parameters.AddWithValue("d", directoryId.HasValue ? (object)directoryId.Value : DBNull.Value);
            cmd.Parameters.AddWithValue("id", id); cmd.ExecuteNonQuery();
        }
        ...
        {
            var conn = new NpgsqlConnection(_cs); conn.Open();
            var cmd = new NpgsqlCommand("DELETE FROM navisworks_projects WHERE id=@id", conn);
            cmd.Parameters.AddWithValue("id", id); cmd.ExecuteNonQuery();
        }
        ...
        public List<ClashTaskModel> GetPendingTasks()
        {
            ...
            var conn = new NpgsqlConnection(_cs); conn.Open();
            var cmd = new NpgsqlCommand(
                "SELECT id,project_id,project_name,nwc_folder,status,error_message,revit_version,created_at,updated_at FROM clash_tasks WHERE status='Pending' ORDER BY created_at", conn);
            var r = cmd.ExecuteReader();
            while (r.Read())
                list.Add(new ClashTaskModel
                {
                    ...
                    Status = Enum.TryParse<ClashTaskStatus>(r.GetString(4), out var s) ? s : ClashTaskStatus.Pending,
        ...
        public void UpdateTaskStatus(string taskId, ClashTaskStatus status, string errorMessage = null)
        {
            ...
            cmd.Parameters.AddWithValue("id", taskId); cmd.ExecuteNonQuery();
        }
        ...
        public void DeleteClashDataForProject(int projectId)
        {
            ...
            cmd.Parameters.AddWithValue("pid", projectId); cmd.ExecuteNonQuery();
        }
        ...
        public void InsertClashResults(List<ClashResultModel> results)
        {
            ...
            foreach (var res in results)
            {
                ...
                cmd.Parameters.AddWithValue("i1id", res.Item1Id ?? ""); cmd.Parameters.AddWithValue("i1n", res.Item1Name ?? "");
                cmd.Parameters.AddWithValue("i1t", res.Item1Type ?? ""); cmd.Parameters.AddWithValue("i1l", res.Item1Layer ?? "");
                cmd.Parameters.AddWithValue("i1sf", res.Item1SourceFile ?? "");
                cmd.Parameters.AddWithValue("i2id", res.Item2Id ?? ""); cmd.Parameters.AddWithValue("i2n", res.Item2Name ?? "");
                cmd.Parameters.AddWithValue("i2t", res.Item2Type ?? ""); cmd.Parameters.AddWithValue("i2l", res.Item2Layer ?? "");
            ...
            int total, int newCount, int active, int reviewed, int approved, int resolved)
        {
            ...
            var cmd = new NpgsqlCommand(@"
                ...
                ON CONFLICT (navisworks_project_id,test_name,record_date) DO UPDATE SET
                    summary_total=EXCLUDED.summary_total, summary_new=EXCLUDED.summary_new,
                    summary_active=EXCLUDED.summary_active, summary_reviewed=EXCLUDED.summary_reviewed,
                    summary_approved=EXCLUDED.summary_approved, summary_resolved=EXCLUDED.summary_resolved,
                    created_at=NOW()", conn);
            cmd.Parameters.AddWithValue("pid", projectId); cmd.Parameters.AddWithValue("tn", testName);
            cmd.Parameters.AddWithValue("tot", total); cmd.Parameters.AddWithValue("new", newCount);
            cmd.Parameters.AddWithValue("act", active); cmd.Parameters.AddWithValue("rev", reviewed);
            cmd.Parameters.AddWithValue("app", approved); cmd.Parameters.AddWithValue("res", resolved);
            cmd.ExecuteNonQuery();
        }

        public bool TestConnection()
        {
            try { var conn = new NpgsqlConnection(_cs); conn.Open(); return true; }
            catch { return false; }
        }
